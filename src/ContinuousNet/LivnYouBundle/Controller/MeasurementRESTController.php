...
use ContinuousNet\LivnYouBundle\Form\MeasurementType;
...
class MeasurementRESTController extends BaseRESTController
{
    ...
    public function postAction(Request $request)
    {
        ...
        if ($form->isValid()) {
            ...
            $authorizedChangeZ350 = false;
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'ADM') > 0) {
                        $authorizedChangeZ350 = true;
                    }
                }
            }
            if (!$authorizedChangeZ350) {
                $entity->setZ350(null);
            }
            ...
            $authorizedChangeTbwffmHcKg = false;
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'ADM') > 0) {
                        $authorizedChangeTbwffmHcKg = true;
                    }
                }
            }
            if (!$authorizedChangeTbwffmHcKg) {
                $entity->setTbwffmHcKg(null);
            }
            $authorizedChangeTbwffmKgRef = false;
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'ADM') > 0) {
                        $authorizedChangeTbwffmKgRef = true;
                    }
                }
            }
            if (!$authorizedChangeTbwffmKgRef) {
                $entity->setTbwffmKgRef(null);
            }
            $authorizedChangeIdffmKg = false;
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'ADM') > 0) {
                        $authorizedChangeIdffmKg = true;
                    }
                }
            }
            if (!$authorizedChangeIdffmKg) {
                $entity->setIdffmKg(null);
            }
            ...
            $authorizedChangeMsRef = false;
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'ADM') > 0) {
                        $authorizedChangeMsRef = true;
                    }
                }
            }
            if (!$authorizedChangeMsRef) {
                $entity->setMsRef(null);
            }
            ...
            $authorizedChangeIcwRef = false;
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'ADM') > 0) {
                        $authorizedChangeIcwRef = true;
                    }
                }
            }
            if (!$authorizedChangeIcwRef) {
                $entity->setIcwRef(null);
            }
            $authorizedChangeIcwi = false;
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'ADM') > 0) {
                        $authorizedChangeIcwi = true;
                    }
                }
            }
            if (!$authorizedChangeIcwi) {
                $entity->setIcwi(null);
            }
            ...
            $authorizedChangeSlmEt = false;
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'ADM') > 0) {
                        $authorizedChangeSlmEt = true;
                    }
                }
            }
            if (!$authorizedChangeSlmEt) {
                $entity->setSlmEt(null);
            }
            ...
            $authorizedChangeEcsEt = false;
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'ADM') > 0) {
                        $authorizedChangeEcsEt = true;
                    }
                }
            }
            if (!$authorizedChangeEcsEt) {
                $entity->setEcsEt(null);
            }
            ...
            $authorizedChangeMpEt = false;
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'ADM') > 0) {
                        $authorizedChangeMpEt = true;
                    }
                }
            }
            if (!$authorizedChangeMpEt) {
                $entity->setMpEt(null);
            }
            ...
            $authorizedChangeFfmEtKg = false;
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'ADM') > 0) {
                        $authorizedChangeFfmEtKg = true;
                    }
                }
            }
            if (!$authorizedChangeFfmEtKg) {
                $entity->setFfmEtKg(null);
            }
            ...
            $authorizedChangeBiodyBluetoothMacAddress = false;
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'ADM') > 0) {
                        $authorizedChangeBiodyBluetoothMacAddress = true;
                    }
                }
            }
            if (!$authorizedChangeBiodyBluetoothMacAddress) {
                $entity->setBiodyBluetoothMacAddress(null);
            }
            $authorizedChangeMachineBluetoothMacAddress = false;
            $roles = $this->getUser()->getRoles();
            if (!empty($roles)) {
                foreach ($roles as $role) {
                    if (substr_count($role, 'ADM') > 0) {
                        $authorizedChangeMachineBluetoothMacAddress = true;
                    }
                }
            }
