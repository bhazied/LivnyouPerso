...
class TranslationCountryRESTController extends BaseRESTController
{
    ...
    {
        try {
            $entity = $this->getDoctrine()->getRepository('LivnYouBundle:TranslationCountry')->get(['id' => $idEntity]);
        ...
        try {
            $data = array(
                'inlineCount' => $this->getDoctrine()
                    ->getRepository('LivnYouBundle:TranslationCountry')
    ...
    {
        try {
            $this->getDoctrine()->getRepository('LivnYouBundle:TranslationCountry')->delete($idEntity);
            return null;
        } catch (\Exception $e) {
            return FOSView::create($e->getMessage(), Response::HTTP_INTERNAL_SERVER_ERROR);
        }
