...
class TemplateRepository extends BaseRepository
{
    ...
    {
        $qBuilder = $this->createQueryBuilder('template');
        $qBuilder->leftJoin('ContinuousNet\LivnYouBundle\Entity\User', 'creator_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'template.creatorUser = creator_user.id');
        $qBuilder->leftJoin('ContinuousNet\LivnYouBundle\Entity\User', 'modifier_user', \Doctrine\ORM\Query\Expr\Join::WITH, 'template.modifierUser = modifier_user.id');
            ...
            if (!empty($memberOfCondition['items'])) {
                if ($memberOfCondition['operator'] == 'or') {
                    ...
                    foreach ($memberOfCondition['items'] as $i => $item) {
                        $orX->add($qBuilder->expr()->isMemberOf(':'.$listName.'_value_'.$i, 'template.'.$listName));
                    ...
                    foreach ($memberOfCondition['items'] as $i => $item) {
                        $andX->add($qBuilder->expr()->isMemberOf(':'.$listName.'_value_'.$i, 'template.'.$listName));
                        $qBuilder->setParameter($listName.'_value_'.$i, $item);
                    }
        ...
        $qBuilder->select('count(template.id)');
